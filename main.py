...
track_dim = 1024  # 원본 임베딩 차원 (실제 값으로 변경 필요)

# 모델 생성 및 로드
clip_model = TitleTrackCLIP(title_dim, track_dim, out_dim=512).to(device)
clip_model.load_state_dict(torch.load("title_track_clip_twostage.pt", map_location=device))
clip_model.eval()
...
async def callback(code: Optional[str] = None):
    ...
    try:
        ...
        if token_data.get("access_token"):
            redirect_url = f"https://dynplayer.win/#access_token={token_data['access_token']}"
...
async def search_songs(request: SearchRequest):
    ...
    try:
        response = supabase.rpc('search_tracks_by_title', {
...
async def recommend(request: RecommendRequest):
    ...
    try:
        response = supabase.rpc('match_tracks_by_key', {
...
@app.post("/recommend-diverse-tracks")
async def recommend_diverse_tracks(request: RecommendDiverseRequest):
    """Diverse 추천"""
    if not request.access_token or not request.spotify_track:
        raise HTTPException(status_code=400, detail="Missing access token or track info")

    try:
        print("🔴 /recommend-diverse-tracks called")

        # 1) Spotify 트랙 정보로 Supabase에서 검색
        title = request.spotify_track.get('name', '')
        artist = request.spotify_track.get('artists', [{}])[0].get('name', '')
        query = f"{title} {artist}".strip()

        search_response = supabase.rpc('search_tracks_by_title', {
            'query_text': query,
            'match_count': 5
        }).execute()

        if not search_response.data or len(search_response.data) == 0:
            print('[diverse] no matching track found in Supabase')
            return {"spotify_tracks": []}

        # 2) 가장 유사도 높은 트랙의 track_key 사용
        seed_track_key = search_response.data[0]['track_key']
        print(f"[diverse] Using seed track_key: {seed_track_key}")

        # 3) 유사한 트랙들 가져오기
        similar_response = supabase.rpc('match_tracks_by_key', {
            'input_track_key': seed_track_key,
            'match_count': 50
        }).execute()

        if not similar_response.data or len(similar_response.data) == 0:
            print('[diverse] no similar tracks found')
            return {"spotify_tracks": []}

        # 4) 유사도 낮은 순으로 정렬 (diverse하게)
        diverse_pool = sorted(similar_response.data, key=lambda x: x.get('similarity', 0))[:15]
        print(f"[diverse] selected {len(diverse_pool)} diverse candidates")

        # 5) Spotify 매핑
        out = []
        async with httpx.AsyncClient() as client:
            for rec in diverse_pool:
                q = f'track:"{rec["title"]}" artist:"{rec["artist"]}"'
        ...
        print(f"[diverse] mapped to Spotify: {len(out)}")
        return {"spotify_tracks": out}

    except Exception as e:
        print(f"recommend-diverse-tracks error: {e}")
        raise HTTPException(status_code=500, detail="Failed to get diverse recommendations")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                if data.get('items'):
                    import random
                    shuffled = random.sample(data['items'], min(len(data['items']), limit))
                    return {"tracks": shuffled}

            # 저장된 트랙이 없으면 추천 트랙 가져오기
            rec_response = await client.get(
                f"https://api.spotify.com/v1/recommendations?seed_genres=pop,rock,electronic&limit={limit}",
                headers={"Authorization": f"Bearer {access_token}"}
            )

            rec_data = rec_response.json()
            return {"tracks": rec_data.get('tracks', [])}

    except Exception as e:
        print(f"random-tracks error: {e}")
...
async def search_by_keyword(request: KeywordSearchRequest):
    """키워드 기반 음악 검색 (OpenAI 임베딩 + CLIP 프로젝션 + Supabase 벡터 검색)"""
    ...
    try:
        ...
        # 3) Supabase에서 벡터 검색 (pgvector 사용)
        # PostgreSQL 함수 호출: match_keyword_embeddings
        response = supabase.rpc('match_keyword_embeddings', {
        ...
        for item in response.data:
            results.append({
                'track_key': item['spotify_track_id'],
                'similarity': item.get('similarity', 0)
            })
...
async def log_listening(request: ListeningLogRequest):
    ...
    if not request.track_name or not request.artist_name:
        raise HTTPException(
            status_code=400,
            detail="Missing required fields: track_name, artist_name"
        )

    try:
        response = supabase.table('listening_logs').insert({
